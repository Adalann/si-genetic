...
class Simulation:
    ...
    def has_converged(self):
        ...
        has_converged = True if self.population.get_global_fitness(
            normalized=True) == 1.0 else False

        if len(self.global_fitness_records) != 0 and not has_converged:
            new_convergence_value = sum(
                self.global_fitness_records) / len(self.global_fitness_records)
